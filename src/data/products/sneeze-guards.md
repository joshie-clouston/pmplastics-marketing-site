---
title: "Sneeze Guards"
description: "Custom acrylic and polycarbonate sneeze guards for retail point of sale — manufactured to any size on the Gold Coast."
headline: "Custom Sneeze Guards — Gold Coast"
features:
  - "Available in acrylic, Perspex, polycarbonate, and plexiglass"
  - "Thickness options: 3mm, 4.5mm, 6mm, and 8mm clear"
  - "Three size configurations including fully custom dimensions"
  - "Protection from airborne contaminants at point of sale"
  - "Durable, easy-to-clean clear finish"
applications:
  - "Retail point of sale counters"
  - "Reception desks"
  - "Food service areas"
  - "Medical and pharmacy counters"
  - "Office front desks"
category: "Commercial"
order: 1
---

## Perspex Sneeze Guards for Retail and Commercial Use

P&M Plastics manufactures all types and sizes of sneeze guards to suit your retail point of sale. Perspex screens provide protection from airborne diseases while offering reassurance to both staff and customers.

### Material Options

We offer sneeze guards in a range of high-quality materials:

- **Acrylic** — lightweight, optically clear, and cost-effective
- **Perspex** — premium branded acrylic with excellent clarity
- **Polycarbonate (Lexan)** — virtually unbreakable for high-traffic environments
- **Plexiglass** — durable thermoplastic with superior light transmission

### Size Configurations

Choose from three options to suit your space:

1. **Size 1 — Small**: Standard small configuration for compact counters
2. **Size 2 — Large**: Standard large configuration for wider service areas
3. **Size 3 — Design Your Own**: Fully custom dimensions to your exact measurements

### Custom Design Specifications

For bespoke sneeze guards, provide the following dimensions in millimetres:

- Base size depth
- Base size width
- Hole size height
- Hole size width
- Overall guard height

### Thickness Options

All guards are available in clear finish with the following thickness options:

- 3mm — lightweight applications
- 4.5mm — standard commercial use
- 6mm — heavy-duty environments
- 8mm — maximum durability

Contact P&M Plastics on **07 5535 7544** for pricing and lead times on your sneeze guard requirements.
